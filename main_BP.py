...
def run_test(test_number, no_epochs, hidden_layers, Export=False):   
    ...
    for epoch in range(no_epochs):
        for i in range(no_training_samples):
            if i % 1000 == 0:
                print('epoch:', epoch, 'img number:', i, '/', no_training_samples)
    ...
    if Export:
        ...
        #plt.clf()
        report = classification_report(test_labels_one_hot, y_test_pred_one_hot, output_dict=True)
        report = DataFrame(report).transpose().values.tolist()
    
        #start CSV file
        ExportToFile="results/BP/"+str(no_training_samples)+str(hidden_layers)+"/"+str(test_number)+"-accuracy-"+str(accuracy)+"-time-"+time_started+".csv"
        with open(ExportToFile, 'a',newline='\n') as out:
            ...
            header = ["#Epochs", "MSE", "Accuracy", "Train_imgs", "Time Elapsed (s)", "Inputs", "Hidden nodes", "Layer 2", "Time started", "Time ended"]
            writer.writerow(header)
            line = [no_epochs, error, accuracy, no_training_samples, time_elapsed, shape[0],shape[1],shape[2], time_started, time_ended]
